...
export default function HeaderNavClient() {
  ...
  return (
    <nav className="flex items-center justify-between w-full max-w-7xl mx-auto gap-4 md:gap-8" aria-label="Main navigation">
      ...
      {/* Desktop nav */}
      <ul id="main-nav-menu" className={`fixed md:static top-16 left-0 w-full md:w-auto bg-white/95 dark:bg-gray-900/95 md:bg-transparent md:dark:bg-transparent shadow-lg md:shadow-none flex-col md:flex-row gap-6 md:gap-8 text-base font-semibold text-gray-700 dark:text-gray-200 md:flex transition-all duration-300 z-40 ${navOpen ? 'flex' : 'hidden md:flex'}`} role="menu">
        ...
        <li><a href="#templates" className="hover:text-indigo-600 dark:hover:text-indigo-400 transition block py-3 md:py-0 px-6 md:px-0" role="menuitem" tabIndex={navOpen ? 0 : -1}>Templates</a></li>
        ...
        <li><a href="#blog" className="hover:text-indigo-600 dark:hover:text-indigo-400 transition block py-3 md:py-0 px-6 md:px-0" role="menuitem" tabIndex={navOpen ? 0 : -1}>Blog</a></li>
      </ul>
