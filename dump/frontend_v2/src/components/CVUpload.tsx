...
import type { CVState } from "@/store/useCVStore";

export default function CVUpload() {
  const inputRef = useRef<HTMLInputElement>(null);
  const abortRef = useRef<AbortController | null>(null);
  // Use selectors with explicit type
  const file = useCVStore((state: CVState) => state.file);
  const setFile = useCVStore((state: CVState) => state.setFile);
  const setData = useCVStore((state: CVState) => state.setData);
  const setFilename = useCVStore((state: CVState) => state.setFilename);
  const setStatus = useCVStore((state: CVState) => state.setStatus);
  const setError = useCVStore((state: CVState) => state.setError);
  const status = useCVStore((state: CVState) => state.status);
  const error = useCVStore((state: CVState) => state.error);
  ...
  const handleFile = async (file: File) => {
    ...
    setShowProcessingTransparency(true); // Show transparency modal
    ...
    try {
      ...
      setTimeout(() => {
        setShowSuccess(true);
        setShowProcessingTransparency(false); // Hide transparency modal
      ...
      setShowProcessingTransparency(false); // Hide transparency modal on error
          ...
          {file && (
            ...
            >
              ...
              >
                ...
                className="flex flex-col gap-3 w-full mt-2 justify-center"
                initial={{ opacity: 0, y: 24 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 1.0, duration: 0.5, type: 'spring' }}
              >
                {/* Primary Action - ATS Resume Maker */}
                <motion.button
                  type="button"
                  className="flex items-center gap-2 px-0 py-0 bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded-lg shadow-md hover:shadow-xl border border-purple-400/30 hover:border-purple-500/60 transition-all duration-300 text-lg font-bold justify-center relative overflow-hidden group h-14 w-full"
                  onClick={() => router.push('/ats-resume')}
                  whileHover={{ scale: 1.02, boxShadow: "0 8px 32px 0 rgba(147,51,234,0.25)" }}
                  whileTap={{ scale: 0.98 }}
                  aria-label="Create ATS Resume"
                >
                  ...
                  <span className="relative z-10">Create ATS Resume</span>
            ...
            >
              ...
              {status === 'error' && file && (
                ...
                >
                  ...
                  {retryParse ? 'Retrying...' : 'Retry Parsing'}              </motion.button>
              )}
            </motion.div>
          )}      </motion.div>
    </GlassCard>
